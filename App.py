...
with c1:
    try:
        ...
        st.markdown("## KB")
with c2:
    st.markdown("## KB VINUELA TRADING")
    st.caption("Diario completo + Escudo + Pilares")
with c3:
    components.html("""
    <div id="reloj" style="background:#000;color:#C9A86A;border:1.5px solid #C9A86A;border-radius:10px;padding:12px;text-align:center;font-weight:800;font-family:serif;"></div>
    <script>
    function tick(){
...
with tab1:
    ...
    with colA:
        ...
        st.info("ORO Soporte 2140 Resistencia 2185")
        with st.expander("ESTRATEGIA FUERTE"):
            st.write("Killzone 14-17h Madrid - Venta ORO 2175-2180 SL 2190 TP 2150")
        with st.expander("ESTRATEGIA CALMA"):
            st.write("Rango 2140-2180 - No operar noticias")
    with colB:
...
with tab3:
    st.subheader("Diario Trading KB")
    with st.form("diario"):
        fecha = st.date_input("Fecha")
        par = st.selectbox("Par", ["ORO / XAUUSD", "DXY", "EURUSD", "GBPUSD"])
        direccion = st.radio("Direccion", ["COMPRA", "VENTA"])
        resultado = st.selectbox("Resultado", ["En espera", "Ganada", "Perdida"])
        notas = st.text_area("Notas")
        if st.form_submit_button("Guardar"):
            st.success(f"Guardado {par} {direccion} {resultado}")
